                ...
                if(malicious_node_index>0)
                    for(int ii=0; ii<malicious_node_index; ++ii)
                    {
                        ...
                        if(rest_of_route.member(tmpID))
                            {
                                return false;
                                route_cache->noticeDeadLink(rest_of_route[0],rest_of_route[rest_of_route.length()-1],Scheduler::instance().clock());
                                route_cache->noticeDeadLink(tmpID,p.dest,Scheduler::instance().clock());
                                route_cache->noticeDeadLink(tmpID,p.src,Scheduler::instance().clock());
                                route_cache->noticeDeadLink(p.dest,tmpID,Scheduler::instance().clock());
                                route_cache->noticeDeadLink(p.src,tmpID,Scheduler::instance().clock());
                                flag=true;
                            }
...
int length=tList->kP_List_New_len_;
  ...
  //MCBDS bellow for changed in MCBDS
...
//      rcm.route.appendToPath(ID(tList->kP_List_New_[i],::IP));
    ...
    if(traceit2==1)
        trace("ACU281: send recheck message to [%s][%d] to_Monitor[%u]. (malicious_node_ID=[%u])",rcm.dest.dump(),destination,(int)srh->recheck_msg_toMonitor(),(int)srh->recheck_msg_maliciousID());
        ...
        */
{
    ...
    else if(srh->recheck_msg_isReply()==1)
    {
        ...
        if(srh->recheck_msg_value()==2)
        {
            ...
            trace("at %f node [%s] detects that [%u] is malicious",Scheduler::instance().clock(),net_id.dump(),received_mal_ID);
                malicious_list_insert(received_mal_ID);
                sendInformPacket(received_mal_ID);
 ...
 // if((int)srh->reply_addrs()[srh->route_reply_len()-1].addr==baitList[myaddr])
  {
            if(traceit2==1)
            {
                    ...
                    trace("reply_route: %s",reply_route.dump());
                            ...
                            /* ==============================================================================
                               ...
                               if(traceit2==1)
                                ...
                                if(tList->kP_len_>=2 && sList->kP_len_>=1)
                                {
                                    ...
                                    kPrimeList_New *tList_new=new kPrimeList_New();
                                    for(int ii=tList->kP_len_-1;ii>=0;--ii)
                                        tList_new->kP_List_New_[tList_new->kP_List_New_len_++]=tList->kP_[ii].addr;

                                    sendRecheckMessage_New(tList_new,(int)sList->kP_[0].addr,(int)reply_route[reply_route.length()-1].addr);
                                ...
                                else if(tList->kP_len_>0 && sList->kP_len_==0)
                                {
                                    ...
                                    */
                                    int mal_node=(int)tList->kP_[1].addr;
                                    trace("ACU281:: Reply received for Bait from [%u]. Length of S List is 0, so we consider sender as Malicious",(int)mal_node);
                                    malicious_list_insert(mal_node);
